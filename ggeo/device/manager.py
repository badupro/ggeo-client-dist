...
class DeviceManager:
    ...
    async def discover(self, _usbmuxd_retry_done: bool = False,
        ...
        try:
            ...
            if (sys.platform == "darwin" and not _usbmuxd_retry_done
                ...
                try:
                    registered = await self._store.list_registered_devices()
                    registered_udids = {d["udid"] for d in registered}
                    ...
                    if missing:
                        ...
                        if await maybe_restart_usbmuxd(source="discover"):
                            ...
                            registered_by_udid = {d["udid"]: d for d in registered}
    ...
    async def _bonjour_discover(self, result: list, found_udids: set):
        """Bonjour WiFi discovery (Windows only) with caching."""
        now = time.time()
    ...
    async def activate(self, udid: str, lat: float, lon: float) -> DeviceSession:
        ...
        if udid in self.sessions:
            raise ValueError("Device %s already active" % self.get_device_name(udid))
